...
class movement(self):
    ...
    def line(t,f=1,L=.2):
        ...
        # TODO: replace these!
        x0 = np.array([0.307, 0, 0.487]) # corresponds to neutral position
        X = np.sqrt((x0[0] + f*t)*(x0[0] + f*t) + 0.487*0.487)
        #X direction
        ...
        def follow_trajectory(self, state, trajectory, final_q):
        ...
        def rotate_claw(camera_tips, Q):
            ...
            if(z_axis is not upward):
                ...
                rotateabouty = [[cos(theta), 0, sin(theta)], [0, 1, 0], [-sin(theta), 0, cos(theta)]] 
                rotateaboutx = [[1, 0, 0], [0, cos(theta), -sin(theta)], [0, sin(theta), cos(theta)]] 
                rotateaboutz = [[cos(theta), -sin(theta), 0], [sin(theta), cos(theta), 0], [0, 0, 1]]
