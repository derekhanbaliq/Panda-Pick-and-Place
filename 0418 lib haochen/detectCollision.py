...
if __name__ == '__main__':
    ...
    for i in range(nline):
        line_pt1 = np.array([[(random.random() * world_length) - world_length / 2,
                              (random.random() * world_length) - world_length / 2,
                              (random.random() * world_length) - world_length / 2]])
        line_pt2 = np.array([[(random.random() * world_length) - world_length / 2,
                              (random.random() * world_length) - world_length / 2,
                              (random.random() * world_length) - world_length / 2]])
        if detectCollision(line_pt1, line_pt2, box)[0]:
            ax.plot#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Edited Shane Rozen-Levy 10/1/20
    Added 3d case
...
if __name__ == '__main__':
    ...
    for i in range(nline):
        ...
        line_pt2 = np.array([[(random.random() * world_length) - world_length / 2,
                              ...
                              (random.random() * world_length) - world_length / 2]])
        if detectCollision(line_pt1, line_pt2, box)[0]:
            ax.plot([line_pt1[0, 0], line_pt2[0, 0]], [line_pt1[0, 1], line_pt2[0, 1]],
                    [line_pt1[0, 2], line_pt2[0, 2]], 'r')
        else:
            ax.plot([line_pt1[0, 0], line_pt2[0, 0]], [line_pt1[0, 1], line_pt2[0, 1]],
                    [line_pt1[0, 2], line_pt2[0, 2]], 'b')
